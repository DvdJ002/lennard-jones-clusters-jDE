...
std::string coordsToString(X x) {
    std::string coordsStr = "( ";
    for (int i = 0; i < x.coords.size(); i++) {
        coordsStr += x.coords[i];
        coordsStr += " ";
    }
    coordsStr += ")";
    return coordsStr;
...
double calculateFitness(std::vector<double> coords, bool optimizeZeroes) {
    // Bool flag determines if optimization will perform
    if (optimizeZeroes) { fixZeroes(coords); }
    double fitness = 0;
    for (int i = 0; i < coords.size()-3; i += 3) {
        for (int j = i + 3; j < coords.size(); j += 3) {
